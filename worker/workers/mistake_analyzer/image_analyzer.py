...
**为什么这样想？** 看到题目中 \( m \)、\( n \) 是方程的根，而要求的式子又包含 \( m^2 + 2019m + 6 \) 这样的二次式，会自然想到：能不能利用"根满足方程"这个性质来简化？
...
**为什么这样想？** 力学和运动学结合题的本质是：力产生加速度，加速度决定运动。所以解题思路自然是"力→加速度→运动"这条主线。
...
**为什么这样想？** 函数图像变换本质上是坐标的系统性改变。理解变换规律后，可以通过跟踪关键点的位置变化来确定新图像。
...
**为什么这样想？** 氧化还原反应的本质是电子转移，所以配平的关键在于保证电子守恒——失去的电子数等于得到的电子数。
...
**为什么这样想？** 看到"等差数列 × 等比数列"的形式，直接求和很困难。但如果能让对应项"错开"后相减，可以消去大部分项，从而简化问题——这就是错位相减法的核心思想。
